...
class Game(object):
    ...
    def load_entries_from_next_timestep(self, is_simulation=False):
        ...
        if self.current_timestep_id == timesteps_ids[-1] and not is_simulation:
            print('cur chronic: {}, cur id: {}'.format(self.__chronic_looper.get_current_chronic_name(), self.current_timestep_id))
            flag = True
            self.__chronic = self.get_next_chronic()
            print('next chronic: ', self.__chronic_looper.get_current_chronic_name())
    ...
    def step(self, action, _is_simulation=False):
        ...
        try:
            ...
            obs, correct_step, done, flag_next_chronic = self.step(action, _is_simulation=_is_simulation)
            ...
            return obs, flag, done, flag_next_chronic  # Return done, not False, because step might diverge

        try:
            # Load next timestep entries, compute one loadflow, then potentially cascading failure
            flag_next_chronic = self.load_entries_from_next_timestep(is_simulation=_is_simulation)
            self._compute_loadflow_cascading(is_simulation=_is_simulation)
        except pypownet.grid.DivergingLoadflowException as e:
            return self.export_observation(), DivergingLoadflowException(e.last_observation, e.text), True, False
        ...
        if np.sum(are_isolated_loads) > self.max_number_loads_game_over:
            ...
            return self.export_observation(), flag, done, flag_next_chronic
        ...
        return self.export_observation(), None, False, flag_next_chronic

    def simulate(self, action, reward_signal):
        ...
        # Step the action
        try:
            simulated_obs, flag, done, _ = self.step(action, _is_simulation=True)
        except pypownet.grid.DivergingLoadflowException as e:
            # Reset previous step
        ...
        #simulated score
        score = reward_signal.get_score(self.grid)
        ...
        return simulated_obs, flag, done, score
