...
class CustomRewardSignal(pypownet.reward_signal.RewardSignal):
    def __init__(self):
        ...
        constant = 0. #worse possible reward possible at one timestep
        ...
        self.multiplicative_factor_distance_initial_grid = 0
        ...
        # Multiplicative factor for the total number of illegal resp. broken lines switches, on-cooldown lines switches
        # and on-cooldown substation switches
        ...
        # Reward when illegal move is performed
        self.illegal_moves_subreward = -constant

        # Action cost reward hyperparameters
        self.multiplicative_factor_number_line_switches = 0 # equivalent to - cost of line switch
        self.multiplicative_factor_number_node_switches = 0 # equivalent to - cost of node switch
    ...
    def compute_reward(self, observation, action, flag):
        ...
        if flag is not None:
            if isinstance(flag, pypownet.environment.DivergingLoadflowException):
                ...
                reward_aslist = [0., 0., self.illegal_moves_subreward, 0., 0.] 
                """
            ...
            line_usage_reward = self.multiplicative_factor_line_usage_reward * np.sum(lines_capacity_usage) #np.square
    ...
    def __get_lines_capacity_usage(observation):
        ampere_flows = np.abs(observation.ampere_flows)
        thermal_limits = np.abs(observation.thermal_limits)
        relative_flow = np.divide(ampere_flows, thermal_limits)

        x = np.maximum(relative_flow, 0)
